...
def show_menu():
    print("=====================WELCOME TO THE GERMAN LANGUAGE PRACTICE PROGRAM==================")
    print("Choose your practice mode:")
    print("1. Word practice")
    print("2. Sentence practice")
    print("3. Translate words")
    print("4. Translate sentences")
    print("5. Pronoun declination pratice")
    print("6. Possessive, reflexive, relative and indefinite pronoun practice")
    mode = input("Enter desired number: ").strip()
    return mode

def color_sentence(doc):
    colored_sentence = ""
    for token in doc:
        color = pos_colors.get(token.pos_, "grey")
        colored_sentence += colored(token.text, color) + " "
    return colored_sentence

known_words_file = "known_words.csv"
unknown_words_file = "unknown_words.csv"
known_sentences_file = "known_sentences.csv"
unknown_sentences_file = "unknown_sentences.csv"

def load_word_list(filename):
    if os.path.exists(filename):
        try:
            return set(pd.read_csv(filename, header=None)[0])
        except pd.errors.EmptyDataError:
            return set()
    return set()

def save_word_list(word_set, filename):
    pd.Series(list(word_set)).to_csv(filename, index=False, header=False)

def load_sentence_list(filename):
    if os.path.exists(filename):
        try:
            return set(pd.read_csv(filename, header=None)[0])
        except pd.errors.EmptyDataError:
...
def save_sentence_list(sentence_set, filename):
    pd.Series(list(sentence_set)).to_csv(filename, index=False, header=False)

def show_known_unknown_words(known_words, unknown_words):
    print("\nKnown words:")
    for word in sorted(known_words):
        ...
        if len(parts) == 2:
            ...
            article, noun = "", word
        color = article_colors.get(article.lower(), "white")
        print((colored(article + " ", color) if article else "") + colored(noun, color) if noun else "")
...
def show_known_unknown_sentences(known_sentences, unknown_sentences):
    print("\nKnown sentences:")
    for sent in sorted(known_sentences):
        doc = nlp(sent)
        print(color_sentence(doc))
    print("\nUnknown sentences:")
    for sent in sorted(unknown_sentences):
        doc = nlp(sent)
        print(color_sentence(doc))
    return None

def ask_word_known(german_word):
    print("Was this word known? (enter/n)")
    answer = input().strip().lower()
    if answer == 'n':
        unknown_words.add(german_word)
        known_words.discard(german_word)
    else:
        known_words.add(german_word)
        unknown_words.discard(german_word)

def ask_sentences_known(german_sentence):
    print("Was this sentence known? (enter/n)")
    answer = input().strip().lower()
    if answer == 'n':
        unknown_sentences.add(german_sentence)
        known_sentences.discard(german_sentence)
    else:
        known_sentences.add(german_sentence)
        unknown_sentences.discard(german_sentence)
...
while True:
    if mode == '1':
        ...
        for line in raw_data.strip().splitlines():
            ...
            if not line:
                continue
            # Match: number. English<TAB>GermanSingular<TAB>GermanPlural (or just "-")
            parts = re.split(r'\s{2,}|\t', line)
            if len(parts) == 3:
                # Sometimes the number and English are together, e.g. "2. Man Der Mann"
        ...
        known_words = load_word_list(known_words_file)
        unknown_words = load_word_list(unknown_words_file)
        ...
        try:
            while True:
                # Weighted sampling: unknown words 3x more likely
                if unknown_words:
                    weights = [3 if row['german_singular'] in unknown_words else 1 for _, row in df_voc.iterrows()]
                else:
                    weights = None
                row = df_voc.sample(n=1, weights=weights).iloc[0]
                ...
                color = article_colors.get(article.lower(), "white")
                colored_german = (colored(article + " ", color) if article else "") + colored(noun, color) if noun else ""

                print("\nGerman:", colored_german)
                print("Show translation? (enter/n)")
                if input().strip().lower() != 'n': 
                    print("English:", english_word)
                ask_word_known(german_word)
        finally:
            # Save lists on exit
            save_word_list(known_words, known_words_file)
            save_word_list(unknown_words, unknown_words_file)
            show_known_unknown_words(known_words, unknown_words)
        mode = show_menu()
    elif mode == '2':
        ...
        print("\nColor legend:")
        print(colored("cyan   ", "cyan")    + ": NOUN/PROPN (nouns, proper nouns)")
        print(colored("green  ", "green")   + ": VERB/AUX (verbs, auxiliary verbs)")
        print(colored("magenta", "magenta") + ": ADJ (adjectives)")
        print(colored("yellow ", "yellow")  + ": ADV/NUM/INTJ (adverbs, numbers, interjections)")
        print(colored("blue   ", "blue")    + ": PRON (pronouns)")
        print(colored("red    ", "red")     + ": ADP/CONJ/CCONJ/SCONJ (prepositions, conjunctions)")
        print(colored("grey   ", "grey")    + ": DET/PART/PUNCT/SYM/X (determiners, particles, punctuation, symbols, other)\n")

        known_sentences = load_sentence_list(known_sentences_file)
        unknown_sentences = load_sentence_list(unknown_sentences_file)

        show_known_unknown_sentences(known_sentences, unknown_sentences)
        try:
            while True:
                # Weighted sampling: unknown words 3x more likely
                if unknown_sentences:
                    weights = [3 if row['german'] in unknown_sentences else 1 for _, row in df_sentences.iterrows()]
                else:
                    weights = None
                row = df_sentences.sample(n=1, weights=weights).iloc[0]
                sentence = row['german']
                doc = nlp(sentence)
                colored_sentence = color_sentence(doc)
                print("German: ", colored_sentence)
                print("Show translation? (enter/n)")
                if input().strip().lower() != 'n':
                    print("English:", row['english'])
                ask_sentences_known(sentence)
        finally:
            save_sentence_list(known_sentences, known_sentences_file)
            save_sentence_list(unknown_sentences, unknown_sentences_file)
            show_known_unknown_sentences(known_sentences, unknown_sentences)
        mode = show_menu()
    elif mode == '3':
        raw_data = """ 
        ...
        show_known_unknown_words(known_words, unknown_words)
        try:
            print("Translate from german to english (1) or from english to german (2)?")
            translation_choice = input().strip().lower() 
            while True:
                # Weighted sampling: unknown words 3x more likely
                if unknown_words:
                    weights = [3 if row['german_singular'] in unknown_words else 1 for _, row in df_voc.iterrows()]
                ...
                if translation_choice == '1':
                    print("\nGerman:", colored_german)
                    print("Write english translation:")
                    answer = input().strip().lower()
                    if answer == english_word.lower() or english_word.lower() in answer:
                        print("Correct!")
                    else:
                        print("Correct answer:", english_word)
                elif translation_choice == '2':
                    print("\nEnglish:", english_word)
                    print("Write german translation:")
                    answer = input().strip().lower()
                    if answer == german_word.lower() or german_word.lower() in answer:
                        print("Correct!")
                    else:
                        print("Correct answer:", german_word)
                ask_word_known(german_word)
        finally:
            # Save lists on exit
            save_word_list(known_words, known_words_file)
        ...
        try:
            ...
            translation_choice = input().strip().lower() 
            while True:
                # Weighted sampling: unknown sentences 3x more likely
                if unknown_sentences:
                    weights = [3 if row['german'] in unknown_sentences else 1 for _, row in df_sentences.iterrows()]
                else:
                    weights = None
                ...
                doc = nlp(german_sentence)
                colored_sentence = color_sentence(doc)
                english_sentence = row['english']
                if translation_choice == '1':
                    print("\nGerman:", colored_sentence)
                    print("Write english translation:")
                    answer = input().strip().lower()
                    if answer == english_sentence.strip().lower() or english_sentence.strip().lower() in answer:
                        print("Correct!")
                    else:
                        print("Correct answer:", english_sentence)
                elif translation_choice == '2':
                    print("\nEnglish:", english_sentence)
                    print("Write german translation:")
                    answer = input().strip().lower()
                    if answer == german_sentence.strip().lower() or german_sentence.strip().lower() in answer:
                        print("Correct!")
                    else:
                        print("Correct answer:", german_sentence)
                ask_sentences_known(german_sentence)
        finally:
            # Save lists on exit
            save_sentence_list(known_sentences, known_sentences_file)
            save_sentence_list(unknown_sentences, unknown_sentences_file)
        ...
        header = (
            f"{'Type':<24} "
            f"{colored('NOM', case_colors['nominative']):<18} "
            f"{colored('ACC', case_colors['accusative']):<18} "
            f"{colored('DAT', case_colors['dative']):<18} "
            f"{colored('GEN', case_colors['genitive']):<18}"
        )
        print(header)
        print("-" * (24 + 18*4))
        for pron_type, forms in german_pronouns.items():
            print(
                f"{pron_type:<24} "
                f"{colored(forms['nominative'], case_colors['nominative']):<18} "
                f"{colored(forms['accusative'], case_colors['accusative']):<18} "
                f"{colored(forms['dative'], case_colors['dative']):<18} "
                f"{colored(forms['genitive'], case_colors['genitive']):<18}"
            )
        print()
        ...
        while True:
            ...
            print(f"\nPronoun type: {pronoun_type}, Case: {colored(case, color)}")
            print(f"German pronoun: {colored(pronoun, color)}")
            ...
            if not matches.empty:
                ...
                print("\nNo sentence found with this pronoun. Skipping.")
                continue
        ...
        german_pronouns = {
        "possessive": [
            {"pronoun": "mein",  "person": "1st singular",       "notes": "my"},
            {"pronoun": "dein",  "person": "2nd singular informal", "notes": "your (informal singular)"},
            {"pronoun": "sein",  "person": "3rd singular masc/neut", "notes": "his / its"},
            {"pronoun": "ihr",   "person": "3rd singular fem / 3rd plural", "notes": "her / their"},
            {"pronoun": "unser", "person": "1st plural",          "notes": "our"},
            {"pronoun": "euer",  "person": "2nd plural informal", "notes": "your (informal plural)"},
            {"pronoun": "Ihr",   "person": "2nd singular & plural formal", "notes": "your (formal)"}
        ],
        "reflexive": [
            {"pronoun": "mich",  "person": "1st singular",       "notes": "myself (accusative)"},
            {"pronoun": "mir",   "person": "1st singular",       "notes": "myself (dative)"},
            {"pronoun": "dich",  "person": "2nd singular informal","notes": "yourself (accusative)"},
            {"pronoun": "dir",   "person": "2nd singular informal","notes": "yourself (dative)"},
            {"pronoun": "sich",  "person": "3rd singular & plural / formal you", "notes": "himself, herself, itself, themselves, yourself (formal)"},
            {"pronoun": "uns",   "person": "1st plural",         "notes": "ourselves"},
            {"pronoun": "euch",  "person": "2nd plural informal","notes": "yourselves"}
        ],
        "relative": [
        ...
        col_width_type = 20
        col_width_pronoun = 20

        header = (
            f"{'Type':<{col_width_type}}"
            f"{colored('POSS', case_colors['possessive']):<{col_width_pronoun}}"
            f"{colored('REFLEX', case_colors['reflexive']):<{col_width_pronoun}}"
            f"{colored('REL', case_colors['relative']):<{col_width_pronoun}}"
            f"{colored('INDEF', case_colors['indefinite']):<{col_width_pronoun}}"
        )
        print(header)
        print("-" * (col_width_type + 4 * col_width_pronoun))

        for pron_type in ["possessive", "reflexive", "relative", "indefinite"]:
            pron_list = german_pronouns[pron_type]
            color = case_colors[pron_type]
            for i, entry in enumerate(pron_list):
                row = ""
                # Only print the type name for the first row of each type
                type_name = pron_type.capitalize() if i == 0 else ""
                row += f"{type_name:<{col_width_pronoun}}"
                # Fill only the correct column
                for col_type in ["possessive", "reflexive", "relative", "indefinite"]:
                    if col_type == pron_type:
                        pronoun = colored(entry["pronoun"], color)
                    else:
                        pronoun = ""
        ...
        while True:
            pronoun_type = random.choice(pronoun_types)
            pronoun_entry = random.choice(german_pronouns[pronoun_type])  # random dict from the list
            pronoun = pronoun_entry["pronoun"]
            color = case_colors[pronoun_type]
            print(f"\nPronoun type: {pronoun_type.capitalize()}")
            print(f"German pronoun: {colored(pronoun, color)}")
